 ...
 * BankingOfficer.java
 *
 * Created on June 13, 2013, 8:38 AM
 */
...
public class BankingOfficer extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        out.println("<html>");
        out.println("<head>");
        out.println("<title>Servlet BankingOfficer</title>");
        out.println("</head>");
        out.println("<body>");
        out.println("<h1>Servlet BankingOfficer at " + request.getContextPath () + "</h1>");
        out.println("</body>");
        out.println("</html>");
         */
        out.close();
        ...
        String obj=request.getParameter("action"); 
	              ...
	              String Code=request.getParameter("bcode");
                      ...
                      String User=request.getParameter("buser");
                       String Pass=request.getParameter("bpassword");
                      Connection con;
	              int rows=0;
	...
	if(obj.equals("Add"))
	    
              {
               ...
               try
		        ...
		        PreparedStatement stat=con.prepareStatement("insert into BankingOfficer values(?,?,?,?)");
		...
		try
                   ...
                   {    Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
                        con=DriverManager.getConnection("jdbc:odbc:Banking","sa","aparnaap");
			PreparedStatement stat=con.prepareStatement("update BankingOfficer set  BOName=?,Username=?,Password=? where BOCode=?");
	...
	if(obj.equals("Delete"))
		    ...
		    {   Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
                        con=DriverManager.getConnection("jdbc:odbc:Banking","sa","aparnaap");
			PreparedStatement stat=con.prepareStatement("delete from BankingOfficer where BOCode=?");
			stat.setString(1,Code);
			rows=stat.executeUpdate();
		    }
		catch(Exception e3)
